...
public class TrainingRunServiceImpl implements TrainingRunService {
	...
	public TrainingRunServiceImpl(TrainingRunRepository trainingRunRepository, AbstractLevelRepository abstractLevelRepository,
			TrainingInstanceRepository trainingInstanceRepository, ParticipantRefRepository participantRefRepository, RestTemplate restTemplate,
	...
	public AbstractLevel getNextLevel(Long trainingRunId) {
		...
		AbstractLevel abstractLevel = abstractLevelRepository.findById(nextLevelId)
				.orElseThrow(() -> new ServiceLayerException("Level with id: " + nextLevelId + " not found.", ErrorCode.RESOURCE_NOT_FOUND));
	...
	public Hint getHint(Long trainingRunId, Long hintId) {
		...
		if (level instanceof GameLevel) {
			Hint hint = hintRepository.findById(hintId)
					.orElseThrow(() -> new ServiceLayerException("Hint with id " + hintId + " not found.", ErrorCode.RESOURCE_NOT_FOUND));
	...
	public int getLevelOrder(Long idOfFirstLevel, Long actualLevel) {
		...
		int order = 0;
		AbstractLevel abstractLevel = abstractLevelRepository.findById(idOfFirstLevel)
				.orElseThrow(() -> new ServiceLayerException("Level with id " + idOfFirstLevel + " not found.", ErrorCode.RESOURCE_NOT_FOUND));
		while (!abstractLevel.getId().equals(actualLevel)) {
			...
			abstractLevel = abstractLevelRepository.findById(abstractLevel.getNextLevel())
					.orElseThrow(() -> new ServiceLayerException("Level with id " + idOfFirstLevel + " not found.", ErrorCode.RESOURCE_NOT_FOUND));
