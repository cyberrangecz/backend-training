...
public class LobbyManagementFacade {
  ...
  public TrainingInstanceLobbyDTO getTrainingInstanceLobby(Long instanceId) {
    ...
    TrainingInstanceLobbyDTO dto =
        trainingInstanceLobbyMapper.mapToDTO(
  ...
  private void addParticipantsToLobbyInstance(TrainingInstanceLobbyDTO instanceLobbyDTO) {
    instanceLobbyDTO.setUsersQueue(
        instanceLobbyDTO.getUsersQueue().stream()
            .map(userRef -> userService.getUserRefDTOByUserRefId(userRef.getUserRefId()))
            .toList());
    instanceLobbyDTO
        .getTeams()
        .forEach(
            teamDTO ->
                teamDTO.setMembers(
                    teamDTO.getMembers().stream()
                        .map(member -> userService.getUserRefDTOByUserRefId(member.getUserRefId()))
                        .toList()));
  }
  ...
  public void returnToQueue(Long instanceId, Set<UserTeamDTO> relations) {
    ...
    relations.stream()
        .map(relation -> userService.getUserByUserRefId(relation.getUserId()))
  ...
  public void disbandTeam(Long teamId) {
    ...
    TrainingInstanceLobby lobby =
        teamsManagementService.getInstanceLobbyOrThrow(team.getTrainingInstance().getId());
  ...
  public void moveUserBetweenTeams(Long teamFrom, Long teamTo, Set<Long> playerIds) {
    ...
    playerIds.stream()
        ...
        .forEach(
            user ->
                this.teamsManagementService.moveUserBetweenTeams(
                    fromTeamEntity, toTeamEntity, user));
  ...
  @PreAuthorize(
      "hasAuthority(T(cz.cyberrange.platform.training.service.enums.RoleTypeSecurity).ROLE_TRAINING_TRAINEE) or "
          + "hasAuthority(T(cz.cyberrange.platform.training.service.enums.RoleTypeSecurity).ROLE_TRAINING_ADMINISTRATOR) or "
  ...
  public String getTrainingInstanceStartDate(String accessToken) {
    return this.trainingInstanceService
        .findByEndTimeBeforeAndAccessToken(accessToken)
        .getStartTime()
  ...
  public TeamDTO getTeamInfo(String accessToken) {
    ...
    if (teamAssigned.isEmpty()) {
      throw new EntityNotFoundException(
          new EntityErrorDetail("This instance has not been accessed"));
    }
