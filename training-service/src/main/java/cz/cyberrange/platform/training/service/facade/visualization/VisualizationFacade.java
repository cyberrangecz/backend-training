...
import cz.cyberrange.platform.training.service.services.IdenticonCombinerService;
...
public class VisualizationFacade {
  ...
  private final IdenticonCombinerService identiconCombinerService;
  ...
  public VisualizationFacade(
      ...
      IdenticonCombinerService identiconCombinerService) {
    ...
    this.identiconCombinerService = identiconCombinerService;
  }
  ...
  public VisualizationProgressDTO getProgressVisualization(Long trainingInstanceId) {
    ...
    playerProgresses =
        addProgressSubjectData(trainingInstance.getType(), trainingInstanceId, playerProgresses);
    visualizationProgressDTO.setProgress(playerProgresses);

    return visualizationProgressDTO;
  }

  private List<ProgressDTO> addProgressSubjectData(
      TrainingType type, Long instanceId, List<ProgressDTO> progresses) {
    return switch (type) {
      case LINEAR -> addLinearProgressSubjectData(instanceId, progresses);
      case COOP -> addCoopProgressSubjectData(instanceId, progresses);
    };
  }

  private List<ProgressDTO> addLinearProgressSubjectData(
      Long instanceId, List<ProgressDTO> progresses) {
  ...
  private List<ProgressDTO> addCoopProgressSubjectData(
      Long instanceId, List<ProgressDTO> progresses) {

    Map<Long, Team> teamsOfInstanceByParticipant =
        trainingInstanceLobbyService.getInstanceLobbyOrThrow(instanceId).getTeams().stream()
            .flatMap(
                team ->
                    team.getMembers().stream()
                        .map(participant -> Map.entry(participant.getUserRefId(), team)))
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    for (ProgressDTO progress : progresses) {
      Team team = teamsOfInstanceByParticipant.get(progress.getId());
      progress.setPicture(
          identiconCombinerService.combineIdenticons(
              team.getMembers().stream()
                  .map(
                      member ->
                          userService.getUserRefDTOByUserRefId(member.getUserRefId()).getPicture())
                  .toList()));
      progress.setName(team.getName());
    }
    return progresses;
  }
