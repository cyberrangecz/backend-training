...
public class TrainingRunService {
  ...
  public TrainingRun deleteTrainingRun(
      Long trainingRunId, boolean forceDelete, boolean deleteDataFromElasticsearch) {
  ...
  private void deleteDataFromElasticsearch(TrainingRun trainingRun) {
    ...
    elasticsearchApiService.deleteEventsFromTrainingRun(
  ...
  public TrainingRun resumeTrainingRun(Long trainingRunId) {
    ...
    validateTrainingRunAccess(trainingInstance, trainingRun);
    auditEventsService.auditTrainingRunResumedAction(trainingRun);
    return trainingRun;
  }

  public void validateTrainingRunAccess(TrainingInstance instance, TrainingRun run) {
    if (run.getState().equals(TRState.FINISHED) || run.getState().equals(TRState.ARCHIVED)) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              TrainingRun.class,
              "id",
    ...
    if (instance.getEndTime().isBefore(LocalDateTime.now(Clock.systemUTC()))) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              TrainingRun.class,
              "id",
    ...
    if (!instance.isLocalEnvironment() && instance.getPoolId() == null) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              TrainingRun.class,
              "id",
    ...
    if (!instance.isLocalEnvironment() && run.getSandboxInstanceRefId() == null) {
      throw new EntityConflictException(
          new EntityErrorDetail(
              TrainingRun.class,
              "id",
  ...
  public boolean checkRunCommandLogging(TrainingRun run) {
    ...
    if (run.getTrainingInstance().isLocalEnvironment()) {
      ...
      runCommands =
          elasticsearchApiService.findAllConsoleCommandsByAccessTokenAndUserId(accessToken, userId);
    } else {
      String sandboxId =
