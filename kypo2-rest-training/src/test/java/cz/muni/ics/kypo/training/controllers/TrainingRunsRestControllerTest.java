...
public class TrainingRunsRestControllerTest {
    ...
    public void isCorrectFlag() throws Exception {
        given(trainingRunFacade.isCorrectFlag(trainingRun1.getId(), "flag")).willReturn(true);
        MockHttpServletResponse result = mockMvc.perform(get("/training-runs/{id}/is-correct", trainingRun1.getId())
        .param("flag", "flag"))
            .andExpect(status().isOk())
            .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
            .andReturn().getResponse();
        assertEquals("true" , result.getContentAsString());
    }
