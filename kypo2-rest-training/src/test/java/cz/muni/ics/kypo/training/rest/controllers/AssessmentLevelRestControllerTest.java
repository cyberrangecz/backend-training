...
public class AssessmentLevelRestControllerTest {
	...
	public void findAssessmentLevelById() throws Exception {
		...
		MockHttpServletResponse result = mockMvc.perform(get("/assessment-levels" + "/{id}", 1l)).andExpect(status().isOk())
	...
	public void findAssessmentLevelByIdWithFacadeException() throws Exception {
		...
		Exception exception =
				mockMvc.perform(get("/assessment-levels" + "/{id}", 1l)).andExpect(status().isNotFound()).andReturn().getResolvedException();
	...
	public void findAllAssessmentLevels() throws Exception {
		...
		MockHttpServletResponse result = mockMvc.perform(get("/assessment-levels")).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)).andReturn().getResponse();
