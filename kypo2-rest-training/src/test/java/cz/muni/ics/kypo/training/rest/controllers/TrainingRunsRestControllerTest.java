...
public class TrainingRunsRestControllerTest {
	...
	public void findTrainingRunById() throws Exception {
		...
		MockHttpServletResponse result = mockMvc.perform(get("/training-runs" + "/{id}", 1l)).andExpect(status().isOk())
	...
	public void findTrainingRunByIdWithFacadeException() throws Exception {
		...
		Exception exception =
				mockMvc.perform(get("/training-runs" + "/{id}", 6l)).andExpect(status().isNotFound()).andReturn().getResolvedException();
	...
	public void findAllTrainingRuns() throws Exception {
		...
		MockHttpServletResponse result = mockMvc.perform(get("/training-runs")).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)).andReturn().getResponse();
