...
public class ClusterMathUtils {
  ...
  public static Double calculateDistance2D(
      Double firstX, Double secondX, Double firstY, Double secondY) {
   ...
   */
  @SafeVarargs
  public static <T extends Clusterable<T>> List<T> normalize(
      List<T> sample, Pair<Function<T, Double>, BiConsumer<T, Double>>... suppliers) {
    for (Pair<Function<T, Double>, BiConsumer<T, Double>> supplierConsumerPair : suppliers) {
      ...
      for (T t : sample) {
        ...
        supplierConsumerPair
            .getSecond()
            .accept(t, (supplierConsumerPair.getFirst().apply(t) - mean) / standardDeviation);
      }
    }
    return sample;
  }
