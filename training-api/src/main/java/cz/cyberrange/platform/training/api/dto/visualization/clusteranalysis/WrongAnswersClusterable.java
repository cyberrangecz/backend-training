...
public class WrongAnswersClusterable implements Clusterable<WrongAnswersClusterable> {
  ...
  public double distanceFrom(WrongAnswersClusterable p) {
    return ClusterMathUtils.calculateDistance2D(
        wrongAnswersSubmittedNormalized,
        p.getWrongAnswersSubmittedNormalized(),
        timePlayedNormalized,
        p.getTimePlayedNormalized());
  }
  ...
  private Double computerWrongAnswersSubmitted(Collection<WrongAnswersClusterable> p) {
    return p.stream()
        .reduce(
            0.0,
            (value, featureOne) ->
                value + featureOne.getWrongAnswersSubmittedNormalized() / p.size(),
            Double::sum);
  }
